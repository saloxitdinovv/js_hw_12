...
for (let i = 0; i < users.length; i++) {
    ...
    let emailEnd = item.email.split('.').at(-1)

    if (emails[emailEnd]) {
        emails[emailEnd].push(item.email)
    } else {
...
// for (let i = 0; i < 1; i++) {
//     let firstFilter = arr.filter((item) => item.completed === true)
//     a.count = a.count + firstFilter.length
//     a.arr.push(firstFilter)

//     let secondFilter = arr.filter((item) => item.completed === false)
